...
class CarModelAdmin(admin.ModelAdmin):
    ...
    list_filter = ['car_make', 'type'] # Fields to create filters for in the sidebar
    search_fields = ['name', 'car_make__name'] # Fields to search by

# CarMakeAdmin class with CarModelInline
class CarMakeAdmin(admin.ModelAdmin):
    """
    Customizes the admin interface for CarMake and includes the CarModelInline.
    """
    list_display = ('name', 'description')
    search_fields = ['name']
    inlines = [CarModelInline] # This is the key line that embeds the CarModel form
